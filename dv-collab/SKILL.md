...
- 验证侧：**不允许阅读 RTL 源码内容，不允许修改 RTL**；以黑盒方式、仅依据
  ...
  侧职责（设计文档应是其文字化）。
  - **行为级阅读的唯一例外**：无设计文档且无设计角色可问时（如单人项目），
    验证侧允许读 RTL，但所得结论一律标注为"推断"，逐条登记进
    `question.yaml`（`source: rtl_inference`，默认 blocking=true），
    待设计口径确认后关闭。
  - **接口信息获取三级**（用户定，能低不高，不构成行为级阅读）：
    1. 默认：只看 spec.yaml 接口段（权威）；
    2. 编译对不上时：查编译报错信息（会给出真实信号名）或仿真器生成的
       接口产物（如 Verilator 模型头文件，只含顶层端口），不打开 RTL 源码；
    3. 仍不够时：登记（文件+原因，记入 `question.yaml` 或 `sim/status.yaml`）
       后只看端口声明区；与 spec 不一致一律登记 `question.yaml`，以 spec
       为准，不得靠读实现反推行为。
  - **flist 用设计侧给的**：`rtl/` 下有设计侧生成并维护的 flist
    （如 `rtl/sim_rtl.f`），仿真编译直接引用它；验证侧不自行扫描
    `rtl/design/` 拼文件列表，缺文件/编译错误反馈设计侧处理。
  - **内部观测例外（细则归 sim-flow 红线节）**：验证侧遇外部端口实在
    无法判定的功能点时，可按 sim-flow『内部观测例外通道』向设计侧询问
    关键内部信号的层级路径；设计侧配合提供路径与含义说明——验证侧不因
    此获得翻阅 RTL 源码的权利，主判定仍以顶层端口为准。
- **唯一豁免目录——`rtl/sim/`（验证专用仿真模型）**：该路径下的 RTL 级
  文件（行为模型、黑盒替代物，如 p380_model.sv）仅供验证使用，**验证侧
  可以查看与修改**；可综合设计代码不放此目录。
- 跨侧访问除本节约定的接口三级与 `rtl/sim/` 豁免外，唯一许可：**用户显式
  指定的文件，且只读不改**；读后结论落入文档，不在对方侧留任何改动。
